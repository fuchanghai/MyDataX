...
public class HdfsHelper {
    ...
    public void deleteFiles(Path[] paths) {
        for (int i = 0; i < paths.length; i++) {
            ...
            try {
                boolean delete = fileSystem.delete(paths[i], true);
                if (delete) {
                    LOG.info("删除成功 ：" + paths[i]);
                }
            } catch (IOException e) {
        ...
        if (tmpFiles.size() != endFiles.size()) {
            ...
            try {
                for (Iterator it1 = tmpFiles.iterator(), it2 = endFiles.iterator(); it1.hasNext() && it2.hasNext(); ) {
                    String srcFile = it1.next().toString().replace("\\\\", "/");
                    String dstFile = it2.next().toString().replace("\\\\", "/");
                    ...
                    if (tmpFilesParent.equals(dstFilePah.getParent())) {
                        canDelete = false;
                    }
