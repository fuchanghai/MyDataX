...
public final class WriterUtil {
    ...
    public static String getWriteTemplate(List<String> columnHolders, List<String> valueHolders, String writeMode, DataBaseType dataBaseType, boolean forceUseUpdate) {
        boolean isWriteModeLegal = writeMode.trim().toLowerCase().startsWith("insert")
                ...
                || writeMode.trim().toLowerCase().startsWith("update") || writeMode.trim().toLowerCase().startsWith("truncate");

        if (!isWriteModeLegal) {
            throw DataXException.asDataXException(DBUtilErrorCode.ILLEGAL_VALUE,
                    String.format("您所配置的 writeMode:%s 错误. 因为DataX 目前仅支持replace,update,truncate 或 insert 方式. 请检查您的配置并作出修改.", writeMode));
        }
        // && writeMode.trim().toLowerCase().startsWith("replace")
    ...
    public static String getWriteTemplateOnTaskGroup(List<String> writeCompareFields, List<String> columnHolders, List<String> valueHolders, String writeMode, DataBaseType dataBaseType, boolean forceUseUpdate, String table) {
        boolean isWriteModeLegal = writeMode.trim().toLowerCase().startsWith("insert")
                || writeMode.trim().toLowerCase().startsWith("replace")
                || writeMode.trim().toLowerCase().startsWith("update") || writeMode.trim().toLowerCase().startsWith("truncate");
        ...
        if (forceUseUpdate ||
            ...
            writeDataSqlTemplate = new StringBuilder()
                    .append(String.format("INSERT INTO %s (", table)).append(StringUtils.join(columnHolders, ","))
                    .append(") VALUES(").append(StringUtils.join(valueHolders, ","))
                    .append(")")
                    .append(onDuplicateKeyUpdateString(columnHolders))
                    .toString();
            ...
            if (writeMode.trim().toLowerCase().startsWith("replace")) {
                ...
                List<String> updateSetValues = new ArrayList<>();
                if (DataBaseType.Oracle == dataBaseType) {
                    List<String> selectValues = new ArrayList<>();
                    for (int i = 0; i < columnHolders.size(); i++) {
                        selectValues.add(valueHolders.get(i) + " as " + columnHolders.get(i));
                    }
                    for (int i = 0; i < writeCompareFields.size(); i++) {
                        joinValues.add("table1." + writeCompareFields.get(i) + " = " + "table2." + writeCompareFields.get(i) + " ");
                    }
                    for (int i = 0; i < columnHolders.size(); i++) {
                        if (!writeCompareFields.contains(columnHolders.get(i))) {
                            updateSetValues.add("table1." + columnHolders.get(i) + " = " + "table2." + columnHolders.get(i));
                        }
                    }
                    writeDataSqlTemplate = new StringBuilder().append("MERGE")
                            .append(String.format(" INTO %s table1 USING( SELECT ", table)).append(StringUtils.join(selectValues, ","))
                            .append(" FROM dual ) table2 ON ( ").append(StringUtils.join(joinValues, " and "))
                            .append(") WHEN MATCHED THEN " + " UPDATE SET ").append(StringUtils.join(updateSetValues, ",")).append(" WHERE ").append(StringUtils.join(joinValues, " and "))
                            .append(" WHEN NOT MATCHED THEN INSERT (").append(StringUtils.join(columnHolders.stream().map(columnHolder -> " table1." + columnHolder).collect(Collectors.toList()), ","))
                            .append(") VALUES(").append(StringUtils.join(columnHolders.stream().map(columnHolder -> "table2." + columnHolder).collect(Collectors.toList()), ",")).append(")").toString();
                } else if (DataBaseType.SQLServer == dataBaseType) {
                    for (int i = 0; i < columnHolders.size(); i++) {
                        if (writeCompareFields.contains(columnHolders.get(i))) {
                            joinValues.add(columnHolders.get(i) + " = " + valueHolders.get(i));
                        }
                    }
                    for (int i = 0; i < columnHolders.size(); i++) {
                        if (!writeCompareFields.contains(columnHolders.get(i))) {
                            updateSetValues.add(columnHolders.get(i) + " = " + columnHolders.get(i));
                        }
                    }
                    writeDataSqlTemplate = new StringBuilder().append(String.format(" if NOT exists(SELECT * FROM %s WHERE  ", table)).append(StringUtils.join(joinValues, " and "))
                            .append(String.format(") INSERT INTO %s ( ", table)).append(StringUtils.join(columnHolders, ","))
                            .append(" ) VALUES(").append(StringUtils.join(valueHolders, ","))
                            .append(String.format(") ELSE UPDATE %s SET ", table)).append(StringUtils.join(updateSetValues, ","))
                            .append(" WHERE ").append(StringUtils.join(joinValues, " and ")).toString();
                } else {
                    //postgresql 和 mysql整一个样
                    for (int i = 0; i < columnHolders.size(); i++) {
                        if (!writeCompareFields.contains(columnHolders.get(i))) {
                            updateSetValues.add(columnHolders.get(i) + " = " + columnHolders.get(i));
                        }
                    ...
                    writeDataSqlTemplate = new StringBuilder().append(String.format(" WITH \"temp\" as (UPDATE %s SET ", table)).append(StringUtils.join(updateSetValues, ","))
                            .append(" WHERE ").append(StringUtils.join(joinValues, " and ")).append(" returning * )")
                            .append(String.format(" INSERT INTO %s (", table)).append(StringUtils.join(columnHolders, ","))
                            .append(" ) select ").append(StringUtils.join(valueHolders, ","))
                            .append(" WHERE (SELECT COUNT(*) from temp) = 0")
                            .toString();
                }
            } else {
                writeDataSqlTemplate = new StringBuilder().append("INSERT")
                        .append(String.format(" INTO %s (", table)).append(StringUtils.join(columnHolders, ","))
                        .append(") VALUES (").append(StringUtils.join(valueHolders, ","))
                        .append(")").toString();
            }
        }
