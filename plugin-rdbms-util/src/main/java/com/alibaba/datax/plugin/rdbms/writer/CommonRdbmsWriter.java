...
public class CommonRdbmsWriter {
    ...
    public static class Task {
        ...
        protected List<String> writeCompareFields;
        ...
        public void init(Configuration writerSliceConfig) {
            ...
            writeCompareFields = writerSliceConfig.getList(Key.WRITE_COMPARE_FIELD, String.class);
            INSERT_OR_REPLACE_TEMPLATE = writerSliceConfig.getString(Constant.INSERT_OR_REPLACE_TEMPLATE_MARK);
            // 如果是group 方式运行的话 那么线程名字就一定不是job-{jobId}
            if (!Thread.currentThread().getName().contains("job")) {
                List<String> valueHolders = new ArrayList<String>(columnNumber);
                for (int i = 0; i < columns.size(); i++) {
                    valueHolders.add(calcValueHolder(""));
                }
                INSERT_OR_REPLACE_TEMPLATE = WriterUtil.getWriteTemplateOnTaskGroup(writeCompareFields, columns, valueHolders, writeMode, dataBaseType, false, this.table);
            }
            this.writeRecordSql = INSERT_OR_REPLACE_TEMPLATE;
        ...
        public void prepare(Configuration writerSliceConfig) {
            ...
            int tableNumber = writerSliceConfig.getInt(
                    Constant.TABLE_NUMBER_MARK, 1);
            if (tableNumber != 1) {
                LOG.info("Begin to execute preSqls:[{}]. context info:{}.",
                        StringUtils.join(this.preSqls, ";"), BASIC_MESSAGE);
                WriterUtil.executeSqls(connection, this.preSqls, BASIC_MESSAGE, dataBaseType);
            } else if (tableNumber == 1 && writeMode.trim().toLowerCase().startsWith("truncate")) {
                if (null == this.preSqls) {
                    this.preSqls = new ArrayList<>();
                }
                this.preSqls.add(String.format("delete from %s",
                        this.table));
                WriterUtil.executeSqls(connection, this.preSqls, BASIC_MESSAGE, dataBaseType);
            }
