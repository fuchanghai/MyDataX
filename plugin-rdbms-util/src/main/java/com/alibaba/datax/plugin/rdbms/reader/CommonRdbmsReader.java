    ...
    public static class Task {
        ...
        public void startRead(Configuration readerSliceConfig,
            ...
            if (StringUtils.isBlank(querySql)) {
                String column = StringUtils.join(readerSliceConfig.getList(Key.COLUMN, String.class), ",");
                readerSliceConfig.set(Key.QUERY_SQL, SingleTableSplitUtil.buildQuerySql(column, table, where));
                querySql = readerSliceConfig.getString(Key.QUERY_SQL);
            } else {
                List<String> constColumns = readerSliceConfig.getList(Key.CONST_FOR_QUERY_SQL, String.class);
               /* if (null != constColumns && constColumns.size() > 0) {
                    for (int i = 0; i < constColumns.size(); i++) {
                        constColumns.set(i, constColumns.get(i) + " as hito_col" + i + " ");
                    }
                }
                String temp = new String(querySql);
                int indexOfFrom = temp.toLowerCase().lastIndexOf("from");
                querySql = querySql.substring(0, indexOfFrom) + "," + StringUtils.join(constColumns, ",") + querySql.substring(indexOfFrom);*/
            }
