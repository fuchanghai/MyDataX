...
public class TaskGroupContainer extends AbstractContainer {
    ...
    public void start() {
        try {
            ...
            LocalTGCommunicationManager.registerTaskGroupCommunication(this.taskGroupId, lastTaskGroupContainerCommunication);
            while (true) {
                //1.判断task状态
    ...
    private List<Configuration> buildRemainTasks(List<Configuration> configurations) {
        ...
        for (int i = 0; i < configurations.size(); i++) {
            Configuration taskConfig = configurations.get(i);
            //taskConfig.set(CoreConstant.TASK_ID, i + 1);
